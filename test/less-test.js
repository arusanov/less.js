...
module.exports = function () {
    ...
    fs.readdirSync(path.join(baseFolder, foldername)).forEach(function (file) {
      ...
      queue(function () {
        toCSS(options, path.join(baseFolder, foldername + file), function (err, result) {
          ...
          if (verifyFunction) {
            const verificationResult = verifyFunction(name, err, result && result.css, doReplacements, result && result.map, baseFolder)
            release()
  ...
  function toCSS (options, path, callback) {
    options = options || {}
    const str = fs.readFileSync(path, 'utf8'), addPath = require('path').dirname(path)
    ...
    if (options.plugin) {
      const Plugin = require(require('path').resolve(process.cwd(), options.plugin))
